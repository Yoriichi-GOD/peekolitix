...
STRICT: Avoid vague language. Use Indian official metrics and cite sources.${disambiguationDirective}`;
    ...
    if (!responseText && process.env.GEMINI_API_KEY) {
      ...
      try {
        ...
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-tts-preview:generateContent?key=${process.env.GEMINI_API_KEY}`, {
          method: 'POST',
